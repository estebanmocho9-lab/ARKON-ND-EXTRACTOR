...
async function processDoc(doc:any,budget:{pages:number;started:number},sheetId:string,knowledge:any[]){const job=await getJob(doc.id,doc.name);if(job.status==='completado'){console.log(`ND | ${doc.name} | ya completado, se omite`);return false;}const neuron=`ND-${String(doc.sourceFolder||'00').padStart(2,'0')}`;console.log(`ND | ${neuron} | ${doc.name} | desde página ${job.nextPage}`);const buffer=await downloadPdf(doc.id);try{const meta=await pdf(buffer);const total=meta.numpages||0;const memory=createMemory(doc.id,doc.name,neuron);await setDocumentStatus(sheetId,doc.id,doc.name,'procesando',0);let next=job.nextPage;let findingsTotal=0;while(next<=total&&budget.pages<MAX_PAGES&&(Date.now()-budget.started)<MAX_MINUTES*60_000){console.log(`ND | ${neuron} | página ${next}/${total} | extracción -> micro-neuronas -> Sheets -> checkpoint`);const result=await deterministicExtract(buffer,next,next);const pages=result.pages||[];const raw=result.findings||[];const neural=processNeuralChunk(raw,memory,knowledge,next,neuron);
    // La unidad transaccional es una sola página: no se avanza ni se hace checkpoint hasta que todas sus escrituras terminan.
    ...
    console.log(`ND | ${neuron} | ${doc.name} | página ${next-1}/${total} CONFIRMADA | hallazgos=${neural.length} | total=${findingsTotal}`);
  }
  if(next>total){await setDocumentStatus(sheetId,doc.id,doc.name,'completado',findingsTotal);await saveJob({driveId:doc.id,name:doc.name,spreadsheetId:sheetId,nextPage:next,totalPages:total,status:'completado'});console.log(`ND | ${neuron} | ${doc.name} | PDF COMPLETADO | paginas=${total} | hallazgos=${findingsTotal}`);}else{await setDocumentStatus(sheetId,doc.id,doc.name,'pausado',findingsTotal);await saveJob({driveId:doc.id,name:doc.name,spreadsheetId:sheetId,nextPage:next,totalPages:total,status:'procesando'});}return true;
  }finally{buffer.fill(0);}}

async function main(){assertNDOnly();const folder=process.env.CARPETA_MADRE_DRIVE_ID;if(!folder)throw new Error('Falta CARPETA_MADRE_DRIVE_ID');const requested=process.env.ND_DOCUMENTO_ID?.trim();const requestedNeuron=process.env.ND_NEURONA?.trim().padStart(2,'0');if(!requested&&!requestedNeuron)throw new Error('Modo seguro: define ND_DOCUMENTO_ID o ND_NEURONA.');const docs=(await listPdfs(folder)).filter(d=>!requestedNeuron||d.sourceFolder===requestedNeuron).filter(d=>!requested||(d.id===requested||d.name.toLowerCase().includes(requested.toLowerCase())));if(!docs.length)throw new Error(requested?`No se encontró el documento: ${requested}`:`No se encontraron PDFs en la neurona ${requestedNeuron}`);const sheetId=await getOrCreateSheet('', 'ND');await ensureNeuralSheets(sheetId);console.log('ND | cargando conocimiento externo en modo SOLO LECTURA...');const knowledge=await loadKnowledge();console.log(`ND | conocimiento de referencia cargado: ${knowledge.length} registros (sin persistencia de extracción)`);const budget={pages:0,started:Date.now()};let processedDocs=0;for(const doc of docs){if(budget.pages>=MAX_PAGES||(Date.now()-budget.started)>=MAX_MINUTES*60_000)break;const did=await processDoc(doc,budget,sheetId,knowledge);if(did){processedDocs++;break;}}
console.log(`ND FIN | PDFs procesados en esta ejecución=${processedDocs} | paginas=${budget.pages} | DB=solo lectura`);}
