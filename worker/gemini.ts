...
export async function extractPdf(pdfPath:string,name:string,documentId:string):Promise<NDFinding[]>{
 const key=process.env.GEMINI_API_KEY;if(!key)throw new Error('Falta GEMINI_API_KEY');
 const manager=new GoogleAIFileManager(key);const uploaded=await manager.uploadFile(pdfPath,{mimeType:'application/pdf',displayName:name});
 const model=new GoogleGenerativeAI(key).getGenerativeModel({model:MODEL});
 const prompt=`${SYSTEM}\nDocumento: ${name}\nDrive ID: ${documentId}\nAnalizá el PDF completo. Devolvé JSON puro según el esquema. La página es obligatoria y debe corresponder a la ubicación real del hallazgo.`;
